...
df = pd.read_csv(r"C:\Users\nandu\Documents\travis\dataset\cleaned_bitext_dataset-f.csv")
...
def generate_response(model, query, max_length=200):
    ...
    # Decode the output
    response = tokenizer.decode(tgt.squeeze(0).tolist())

    # Add newline after sentence-ending periods, but skip list item numbers like "1.", "2.", etc.
    formatted_response = re.sub(r'(?<!\b\d)\.(\s+)', r'.\n', response)
